...
O parágrafo 95 (§5.4, Figura 5.12) trazia medições de escorregamento que
**foram removidas do texto**, em três passadas no mesmo dia:

1. **Ângulo acumulado / voltas** ("6 916°, equivalentes a 19,2 voltas completas
   em 300 ms"): grandeza integrada, sem critério de classificação ("quantas
   voltas é demais?"), a mesma objeção que tirou o ISE do dashboard (ver
   [[cards-metricas]]). Depende do comprimento da janela (os 300 ms são só o
   tempo de parada da simulação). Notação `6.916°` ainda ambígua.
2. **Estabilização / "regime permanente"**: a janela de 300 ms não caracteriza
   regime estabelecido — a taxa oscila entre 65 e 81 Hz e o erro de fase cresce
   continuamente.
3. **Frequência de escorregamento (~70 Hz)**: decisão do usuário — parâmetro que
   não sustenta nenhuma conclusão da seção e soa como "frequência do erro de
   fase", abrindo flanco a questionamento. Sem argumento forte para manter, foi
   retirado.
...
Os valores medidos ficam nas duas linhas da tabela acima e em
`output/medicao_escorregamento_srf_pll.pdf` / [[pll-cycle-slip-measurement]],
como registro de método — órfãos em relação ao texto, mas a nota é documento de
apoio, não vai no TCC.
