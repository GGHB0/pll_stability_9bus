...
| `pll_control_loop.svg` / `.png` | SVG (controle) | Figura 4.3 do fragmento do TCC — blocos do laco SRF-PLL: Park, PI (Kp+Ki/s), feed-forward de omega_n, somador, integrador 1/s (VCO), realimentacao de fase. **Refeito em 2026-08-23**: viewBox de 920x340 para 680x350 e fontes de 13-18 px, porque no layout antigo o texto chegava a pagina do DOCX com 4,9 pt; `phi_chapeu` trocado por `theta_PLL` para casar com `vsi_lcl_pwm_circuit` | `.claude/kb/pll/srf_pll_theory.md`, `.claude/kb/pll/pll_gains_methodology.md` |
| `matriz_cenarios.svg` / `.png` | SVG (matriz) | Figura 4.4 do fragmento do TCC — matriz local da falta x tipo (1/2/3 fases) x sintonia do PLL, com marca N (nominal) e I (inadequada) por celula, mais a linha de regime permanente. **Nao desenhado a mao**: gerado por `scripts/gen_matriz_cenarios.py`, que le `output/results/` direto — reexecutar apos qualquer re-simulacao | `.claude/kb/simulation/cenarios_simulados.md` |
...
| `vsi_grid_schematic.svg` / `.png` | SVG (blocos) | Figura 2.1 do TCC — esquematico generico VSI conectado a rede (fonte CC -> VSI -> filtro LCL -> PCC -> rede), com bloco de controle digital abstrato | `.claude/kb/tcc-word/content_map.md` |
